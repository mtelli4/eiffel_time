...
function startGeneratingQRCode() {
    // Fct pour démarrer la génération de code QR
...
async function stopGeneratingQRCode() {
    // Fct pour arrêter la génération de code QR
...
function simulateScan() {
    // Fct pour simuler un scan réussi
    const name = document.getElementById('name').value
    if (name) {
        const now = new Date()
...
function addToScanList(name, status) {
    // Fct pour ajouter un nom à la liste des scans
