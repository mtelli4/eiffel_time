...
function comparePasswords(firstPassword: string[], secondPassword: string[]): boolean {
    const password1 = firstPassword[0];
    const salt1 = firstPassword[1];
    const password2 = secondPassword[0];
    const salt2 = secondPassword[1];
    return hashPassword(password1, salt1).hash === hashPassword(password2, salt2).hash;
}

function isPasswordValid(password: string): { isValid: boolean; errors: string[] } {
    let errors: string[] = [];
